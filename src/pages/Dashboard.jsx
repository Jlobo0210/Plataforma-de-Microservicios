    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {['all', 'active', 'building', 'disabled'].map(f => (
          ...
          >
            {{ all: 'Todos', active: 'Activos', building: 'Building', disabled: 'Off' }[f]}
          </button>
        ))}
      </div>
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(s => (
            <ServiceCard key={s.id} service={s} onDelete={onDelete} onToggle={onToggle} />
          ))}
        </div>
      ) : (
        <div className="flex flex-col items-center justify-center py-20 text-slate-500 gap-3">
          <p className="text-lg">Sin resultados</p>
          <p className="text-sm">Crea tu primer microservicio o ajusta los filtros</p>
        </div>
      )}
    </div>
  );
