...
const EMPTY = { name: '', description: '', language: 'PYTHON', sourceCode: '' };
...
export default function CreateServiceModal({ isOpen, onClose, onSubmit }) {
  ...
  const validate = () => {
    ...
    if (!/^[a-z0-9-]+$/.test(form.name)) e.name = 'Solo minúsculas, números y guiones';
    if (!form.description.trim())       e.description = 'Requerido';
    if (!form.sourceCode.trim())        e.sourceCode = 'Pega el código fuente';
    return e;
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="w-full max-w-xl max-h-[90vh] overflow-y-auto bg-slate-900 rounded-2xl border border-slate-700 shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700 sticky top-0 bg-slate-900">
          <h2 className="text-base font-bold text-slate-100">Nuevo Microservicio</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          {/* Nombre */}
          <div>
            <label className="text-sm text-slate-300 font-medium">Nombre <span className="text-cyan-400">*</span></label>
          ...
          {/* Descripción */}
          <div>
            <label className="text-sm text-slate-300 font-medium">Descripción <span className="text-cyan-400">*</span></label>
            <textarea value={form.description} onChange={e => set('description', e.target.value)}
              rows={2} placeholder="¿Qué hace este microservicio?"
          ...
          {/* Código */}
          <div>
            <label className="text-sm text-slate-300 font-medium">Código fuente <span className="text-cyan-400">*</span></label>
            <div className="mt-1">
              <CodeEditor value={form.sourceCode} onChange={v => set('sourceCode', v)} language={form.language} />
            </div>
            {errors.sourceCode && <p className="text-xs text-red-400 mt-1">{errors.sourceCode}</p>}
          </div>

          {/* Botones */}
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-lg border border-slate-700 text-slate-400 text-sm hover:text-slate-200 transition-colors">
              Cancelar
            </button>
